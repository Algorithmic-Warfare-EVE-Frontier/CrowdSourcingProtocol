...
ResourceId constant CSMotionsTableTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x00b5070120142020202001000000000000000000000000000000000000000000
);
...
library CSMotionsTable {
    ...
    )
  {
    ...
    target = (address(Bytes.slice20(_blob, 32)));

    momentum = (uint256(Bytes.slice32(_blob, 52)));

    push = (uint256(Bytes.slice32(_blob, 84)));

    pull = (uint256(Bytes.slice32(_blob, 116)));

    lifetime = (uint256(Bytes.slice32(_blob, 148)));

    status = MotionStatus(uint8(Bytes.slice1(_blob, 180)));
  }
