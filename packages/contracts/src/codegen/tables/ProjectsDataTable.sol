...
library ProjectsDataTable {
  ...
  FieldLayout constant _fieldLayout =
    FieldLayout.wrap(0x0025030220040100000000000000000000000000000000000000000000000000);

  // Hex-encoded key schema of (bytes32)
  Schema constant _keySchema = Schema.wrap(0x002001005f000000000000000000000000000000000000000000000000000000);
  // Hex-encoded value schema of (uint256, uint32, uint8, string, string)
  Schema constant _valueSchema = Schema.wrap(0x002503021f0300c5c50000000000000000000000000000000000000000000000);
  ...
  function decodeStatic(
    ...
    balance = (uint256(Bytes.getBytes32(_blob, 0)));

    rejections = (uint32(Bytes.getBytes4(_blob, 32)));

    projectStatus = ProjectStatus(uint8(Bytes.getBytes1(_blob, 36)));
  }
