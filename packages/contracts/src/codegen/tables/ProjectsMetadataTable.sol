...
library ProjectsMetadataTable {
  ...
  FieldLayout constant _fieldLayout =
    FieldLayout.wrap(0x005c050004202004140000000000000000000000000000000000000000000000);

  // Hex-encoded key schema of (bytes32)
  Schema constant _keySchema = Schema.wrap(0x002001005f000000000000000000000000000000000000000000000000000000);
  // Hex-encoded value schema of (uint32, uint256, uint256, uint32, address)
  Schema constant _valueSchema = Schema.wrap(0x005c0500031f1f03610000000000000000000000000000000000000000000000);
  ...
  function decodeStatic(
    ...
    timestamp = (uint32(Bytes.getBytes4(_blob, 0)));

    threshold = (uint256(Bytes.getBytes32(_blob, 4)));

    target = (uint256(Bytes.getBytes32(_blob, 36)));

    deadline = (uint32(Bytes.getBytes4(_blob, 68)));

    manager = (address(Bytes.getBytes20(_blob, 72)));
  }
