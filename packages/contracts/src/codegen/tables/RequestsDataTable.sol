...
// Hex below is the result of `WorldResourceIdLib.encode({ namespace: "csp", name: "RequestsDataTabl", typeId: RESOURCE_TABLE });`
ResourceId constant _tableId = ResourceId.wrap(0x746263737000000000000000000000005265717565737473446174615461626c);
ResourceId constant RequestsDataTableTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x000a040104040101000000000000000000000000000000000000000000000000
);
...
library RequestsDataTable {
  ...
  {
    approvalRate = (uint32(Bytes.slice4(_blob, 0)));

    denialRate = (uint32(Bytes.slice4(_blob, 4)));

    requestStatus = RequestStatus(uint8(Bytes.slice1(_blob, 8)));

    approvalStatus = ApprovalStatus(uint8(Bytes.slice1(_blob, 9)));
  }
